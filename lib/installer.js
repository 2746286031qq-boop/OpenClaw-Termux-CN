...
export function setupProotUbuntu() {
  ...
  const setupScript = `
    ...
    node /root/.openclaw/dashboard-zh-patch.mjs
  `;
...
`;
  ...
  // Deploy dashboard Chinese patch (read from project lib dir at build time)
  const dashSrc = path.join(import.meta.dirname, 'dashboard-zh-patch.mjs');
  if (fs.existsSync(dashSrc)) {
    try {
      const dashPath = path.join(PROOT_UBUNTU_ROOT, '.openclaw', 'dashboard-zh-patch.mjs');
      const dashDir = path.dirname(dashPath);
      if (!fs.existsSync(dashDir)) fs.mkdirSync(dashDir, { recursive: true });
      fs.writeFileSync(dashPath, fs.readFileSync(dashSrc), 'utf8');
    } catch (err) {
      console.error('Failed to deploy dashboard ZH patch:', err.message);
    }
  }
